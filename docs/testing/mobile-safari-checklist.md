...
| E4  | Emergency exit everywhere  | From the reconnect screen, the play shell, and the lobby, use Leave / End-game-for-everyone.                                                         | Every control works on the phone, always reachable (never inside the game frame), and ends/leaves cleanly for everyone.                                                                                   | P4 play-shell tests; engine cleanup tests                                  |
...
| F2  | Several simultaneous test frames         | In the arena (`/test`), run Nova Quiz with 4 players at once on the phone.     | All four frames boot, register, and stay responsive; controls remain tappable; suspending/resuming one player does not disturb the others.     | arena e2e (4 frames, headless); F4 device pass |
