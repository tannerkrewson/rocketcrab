...
| WebSocket                       | ✅ pass (local wss echo)                | text echo round-trip on `wss://localhost:5276`. _Public echo service (echo.websocket.events) was DNS-unreachable from this network._     | ⏳ probe added 2026-08-01 (re-load hello game to exercise; echo on `:5276`)       |
...
| Fullscreen                      | ❌ headless unsupported (informational) | `requestFullscreen` → `false`; requires headed/real browser                                                                              | ❌ unsupported on iPhone Safari (platform limitation, not a spike defect)         |
| Pointer lock                    | ❌ headless unsupported (informational) | `requestPointerLock` → `false`; requires headed/real browser                                                                             | ❌ unsupported on iPhone Safari (platform limitation)                             |
...
| CPU exhaustion / infinite loop  | ⚠️ environment-dependent                | see findings: no OOPIF in headless shell → whole tab wedges; recovery via browser-level teardown; **shell survival needs real browsers** | ❌ **FAIL** — whole Safari tab wedges; Emergency stop unresponsive (see findings) |
