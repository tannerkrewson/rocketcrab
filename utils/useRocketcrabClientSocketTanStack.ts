/**
 * TanStack Router-compatible version of useRocketcrabClientSocket.
 *
 * Same logic as the original, but accepts a generic "router-like" object
 * instead of NextRouter, so it can be used without importing next/router.
 */
...
/**
 * Minimal router interface that TanStack Router satisfies.
 */
...
export const useRocketcrabClientSocket = ({
    ...
    useEffect(() => {
        ...
        // "code" will be undefined during initial render
        if (code && !partyState) {
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [code, partyState, isReconnect, socketConnected, cookiePartyState]);
